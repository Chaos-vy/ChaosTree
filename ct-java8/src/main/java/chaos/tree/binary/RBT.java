...
import static chaos.tree.binary.Color.BLACK;
import static chaos.tree.binary.Color.RED;
 ...
 * @since 1.0.0
 */
public final class RBT<T extends Comparable<? super T>> extends AbstractRotateTree<T, RBTNode<T>> {
    ...
    /*
     * Came to read the RBT tree deletion. Well it is not hard well really hard..... If you came for contribution there is
     * important note too it must be changed to improve if and only if it beats the old benchmark. Correctness first
     * then the benchmark. I am taking a gap for this. Till now, it is on bleeding edge with TreeSet.
     */
    @Override
