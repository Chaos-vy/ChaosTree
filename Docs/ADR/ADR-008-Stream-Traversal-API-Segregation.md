...
When we started building our high-performance N-ary trees (specifically the `BPlusTree`), this design suddenly became a fatal flaw. `BPlusTree`s use "Ghost Routers"—deleted keys that are kept around strictly to route traffic. If you ran a structural Pre-order traversal on a `BPlusTree`, it would spit out all these deleted ghost keys and duplicate routing data. It completely violated the fundamental contract of what a Search Tree is supposed to do.

We fixed this by leaning hard into the **Interface Segregation Principle (ISP)**. We defined a strict, absolute baseline contract in `ISearchTree`, and then allowed specific tree families to add their own specialized methods based on what they are mathematically good at.

We were basically staring down a massive API contradiction:
...
Here is how we broke up the API:
...
* **Pros:** Total mathematical purity. We never have to throw an `UnsupportedOperationException`, and we never violate our own data contracts.
...
* **Resolution:** We preserve the Liskov Substitution Principle perfectly. If a user just wants their data sorted, the base interface gives it to them. If they specifically need to do a structural Pre-order serialization, they should be specifically asking for a `BinaryTree` anyway.

Our API stays perfectly consistent across completely different data structures, and we don't have to rely on ugly `UnsupportedOperationException` hacks.
