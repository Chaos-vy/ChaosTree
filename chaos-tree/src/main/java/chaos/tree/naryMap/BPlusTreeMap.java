...
public final class BPlusTreeMap<K, V> extends AbstractNaryTreeMap<K, V, BPlusTreeMapNode<K, V>> {
    ...
    public void buildFromSorted(Iterator<? extends Map.Entry<? extends K, ? extends V>> it, float factor) {
        ...
        if (rightEdge[0] != null && rightEdge[0] != this.root && rightEdge[0].keyCount < minKeys) {
            ...
            if (childIdx > 0) {
                ...
                if (node.keyCount < minKeys) {
                    ...
                    if (node.next != null) node.next.prev = leftSib;
        ...
        while (root.keyCount == 0 && !root.isLeaf()) {
            root = root.child[0];
            root.parent = null;
        }
        this.modCount++;
    ...
    private abstract class BPlusTreeBaseIterator<T> implements Iterator<T> {
        long expectedModCount;
        BPlusTreeMapNode<K, V> currentLeaf;
        int currentIndex;
        K lastReturnedKey = null;
        BPlusTreeMapNode<K, V> lastReturnedLeaf = null;
        int lastReturnedIndex = -1;

        BPlusTreeBaseIterator(K startKey, boolean startInclusive) {
        ...
        public final void remove() {
            ...
            final BPlusTreeMapNode<K, V> leaf = lastReturnedLeaf;
            ...
            if (leaf.keyCount == oldCount - 1) {
                if (idx0 < leaf.keyCount) {
                    ...
                    BPlusTreeMapNode<K, V> n = leaf.next;
    ...
    private final class EntryIterator extends BPlusTreeBaseIterator<Map.Entry<K, V>> {
    ...
    private final class KeyIterator extends BPlusTreeBaseIterator<K> {
    ...
    private final class ValueIterator extends BPlusTreeBaseIterator<V> {
    ...
    private abstract class BPlusTreeReverseBaseIterator<T> implements Iterator<T> {
        long expectedModCount;
        BPlusTreeMapNode<K, V> currentLeaf;
        int currentIndex;
        K lastReturnedKey = null;
        ...
        BPlusTreeReverseBaseIterator(K startKey, boolean startInclusive) {
            this.expectedModCount = modCount;
            if (root == null) return;
        ...
        public final void remove() {
            ...
            final BPlusTreeMapNode<K, V> leaf = lastReturnedLeaf;
            final int oldCount = leaf.keyCount;
            final BPlusTreeMapNode<K, V> curLeaf = currentLeaf;
            final int curIdx = currentIndex;

            removeAtLeaf(leaf, lastReturnedIndex);
            expectedModCount = modCount;
            lastReturnedKey = null;
            lastReturnedLeaf = null;
            ...
            if (leaf.keyCount == oldCount - 1
                    && curIdx < curLeaf.keyCount
                    && curLeaf.keys[curIdx] == nextTarget) {
                currentLeaf = curLeaf;
                currentIndex = curIdx;
                return;
            }
    ...
    private final class ReverseEntryIterator extends BPlusTreeReverseBaseIterator<Map.Entry<K, V>> {
