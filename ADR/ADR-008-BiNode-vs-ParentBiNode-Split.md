# ADR-008: BiNode vs ParentBiNode Split (Parent Pointer Separation)

Building up the Red-Black Tree (`RBT`), I encountered the problem with the deletion algorithm. The fixup routine (`fixDoubleBlack`) requires me to traverse *upward*—from a node back to its parent, and then to its grandparent. 
...
But RBT can't do that. Its fixup routine needs to find the *sibling* of a node, and to find a sibling, you must know the parent. There's no advance call-stack trick that substitutes for having an explicit parent reference when you're jumping around during a non-recursive fixup. The exact same issue applied to my `Splay` tree—its zig-zig and zig-zag rotations require parent awareness.
...
Node byte sizes confirmed via GC profiling (`-prof gc`) and `JOL`:
