# ADR-006: flatTree Retired

After RBT tree completion phase, I tried building a 'Flat Tree'. Instead of nodes holding actual memory pointers (`left`, `right`) to their children, the entire binary tree was mapped perfectly onto a single, contiguous 1D array (where the root is at index 0, the left child at $2i+1$, and the right child at $2i+2$).
