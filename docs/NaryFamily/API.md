...
The `NaryTree<T>` interface inherits from `ISearchTree<T>` and `Tree`. I designed it to give you a comprehensive set of operations specifically optimized for multi-way search trees like B-Trees and B+ Trees.
...
| `boolean containsAllElements(Iterable<? extends T> values)` | Returns `true` if all values exist in the tree.                  | `NullPointerException` (if any null)                           |
| `void retainAllElements(Iterable<? extends T> values)`      | Retains only elements present in both the tree and the iterable. | `NullPointerException` (if iterable is null)                   |
