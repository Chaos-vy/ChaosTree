...
public final class BTreeMap<K, V> extends AbstractNaryTreeMap<K, V, BTreeMapNode<K, V>> {
    ...
    protected Iterator<Map.Entry<K, V>> entryIterator(K fromKey, boolean fromInclusive) {
        return new BTreeIterator(fromKey, fromInclusive);
    }

    @Override
    protected Iterator<Map.Entry<K, V>> descendingEntryIterator(K fromKey, boolean fromInclusive) {
        return new BTreeReverseIterator(fromKey, fromInclusive);
    }

    private final class BTreeIterator implements Iterator<Map.Entry<K, V>> {
        private long expectedModCount;
        private BTreeMapNode<K, V> currentNode;
        private int currentIndex;

        BTreeIterator(K startKey, boolean startInclusive) {
    ...
    private final class BTreeReverseIterator implements Iterator<Map.Entry<K, V>> {
        private long expectedModCount;
        private BTreeMapNode<K, V> currentNode;
        private int currentIndex;

        BTreeReverseIterator(K startKey, boolean startInclusive) {
            this.expectedModCount = modCount;
            if (root == null) return;
